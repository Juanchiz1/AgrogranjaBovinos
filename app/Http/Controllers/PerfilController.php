   ...
   public function update(Request $request)
{
    $uid  = session('usuario_id');
    $user = DB::table('usuarios')->find($uid);
    $tab  = $request->tab_actual ?? 'datos';

    // Solo validar nombre cuando se guarda desde el tab de datos personales
    if ($tab === 'datos') {
        $request->validate(['nombre' => 'required']);
    }

    $data = ['actualizado_en' => now()->toDateTimeString()];

    // Campos del tab datos
    if ($tab === 'datos') {
        ...
        try { $data['rut'] = $request->rut; } catch (\Exception $e) {}
    }

    // Campos del tab finca
    if ($tab === 'finca') {
        $data = array_merge($data, [
    ...
    return redirect()->route('perfil.index', ['tab' => $tab])
        ->with('msg', 'Guardado correctamente.')
