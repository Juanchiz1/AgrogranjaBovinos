...
class AnimalController extends Controller
{
    ...
    public function show($id)
    {
        ...
        $inventarioItems = DB::table('inventario')
    ->where('usuario_id', $uid)
    ->where('cantidad_actual', '>', 0)
    ->orderBy('categoria')->orderBy('nombre')->get();

$compradores = DB::table('personas')
    ->where('usuario_id', $uid)->where('activo', 1)
    ->whereIn('tipo', ['comprador','cliente','contacto','otro'])
    ->orderBy('nombre')->get();
        ...
        return view('pages.animal-detalle', compact(
            ...
            'valorVentaEst', 'emojis', 'especies', 'personas', 'genealogia','inventarioItems','compradores',
        ));
    }
...
if ($request->inventario_id && $request->inventario_cantidad > 0) {
    ...
    if ($insumo) {
        ...
        DB::table('inventario')->where('id', $insumo->id)
            ->decrement('cantidad_actual', $request->inventario_cantidad);
    }
}
