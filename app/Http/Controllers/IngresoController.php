    ...
    private function tipos(): array
{
    return [
        // Ventas
        'venta'            => ['label' => 'Venta directa',              'emoji' => '💰'],
        'cosecha_propia'   => ['label' => 'Cosecha / Producción',       'emoji' => '🌾'],
        'animal'           => ['label' => 'Venta de animal',            'emoji' => '🐄'],
        // Subproductos
        'leche'            => ['label' => 'Venta de leche',             'emoji' => '🥛'],
        'huevos'           => ['label' => 'Venta de huevos',            'emoji' => '🥚'],
        'subproducto'      => ['label' => 'Otro subproducto',           'emoji' => '📦'],
        // Servicios y otros ingresos
        'servicio'         => ['label' => 'Servicio prestado',          'emoji' => '🔧'],
        'alquiler'         => ['label' => 'Alquiler de tierra/equipo',  'emoji' => '🏡'],
        'subsidio'         => ['label' => 'Subsidio / Ayuda gobierno',  'emoji' => '🏛️'],
        'prestamo'         => ['label' => 'Préstamo / Crédito',         'emoji' => '🏦'],
        'devolucion'       => ['label' => 'Devolución / Reembolso',     'emoji' => '↩️'],
    ...
    try {
        DB::table('cultivo_eventos')->insert([
            ...
            'usuario_id'  => $uid,
            'tipo'        => 'nota',
            'titulo'      => '💵 Ingreso: ' . $request->descripcion,
            'descripcion' => '$' . number_format($total, 0, ',', '.') .
                             ($comprador ? ' · ' . $comprador : '') .
                             ($request->tipo ? ' · ' . ($this->tipos()[$request->tipo]['label'] ?? $request->tipo) : ''),
            'fecha'       => $request->fecha ?? now()->toDateString(),
...
// ── Registrar en línea de tiempo del animal ───────────────────────
...
// ── Marcar cosecha como con ingreso creado ───────────────────────
if ($request->cosecha_id) {
    try {
        DB::table('cosechas')->where('id', $request->cosecha_id)
