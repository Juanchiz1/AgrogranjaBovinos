...
// ── Registrar labor en persona_labores ───────────────────────────
if ($request->persona_id) {
    DB::table('persona_labores')->insert([
        ...
        'cultivo_id'  => $id,
        'notas'       => $request->descripcion ?: null,
        'creado_en'   => now()->toDateTimeString(),
    ]);
}        
      

        // ── Descontar insumo del inventario ──────────────────────────────
if ($request->inventario_id && $request->inventario_cantidad > 0) {
    ...
    if ($insumo) {
        DB::table('inventario_movimientos')->insert([
            ...
            'cultivo_id'     => $id,
            'tipo'           => 'salida',
            'cantidad'       => $request->inventario_cantidad,
            'precio_unitario'=> $insumo->precio_unitario,
            'motivo'         => $request->titulo,
        ...
        DB::table('inventario')->where('id', $insumo->id)->decrement(
            'cantidad_actual', $request->inventario_cantidad
        );
    }
}

// ── Generar gasto automáticamente ────────────────────────────────
if ($request->generar_gasto && $request->gasto_valor > 0) {
    DB::table('gastos')->insert([
        'usuario_id'  => $uid,
        'cultivo_id'  => $id,
        'persona_id'  => $request->persona_id ?: null,
        'categoria'   => match($request->tipo) {
            'fertilizacion'          => 'Fertilizantes',
            'aplicacion_agroquimico' => 'Agroquímicos',
            'riego'                  => 'Riego',
            'control_fitosanitario'  => 'Fitosanitarios',
            'deshierbe'              => 'Mano de obra',
            default                  => 'Insumos',
        },
        'descripcion' => $request->titulo,
        'valor'       => $request->gasto_valor,
        'fecha'       => $request->fecha,
        'proveedor'   => $request->gasto_proveedor ?: null,
        'notas'       => 'Generado desde evento de cultivo',
        'creado_en'   => now()->toDateTimeString(),
    ]);
}
