...
<div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid var(--border);">
  <div style="width:34px;height:34px;border-radius:50%;background:var(--verde-bg);display:flex;align-items:center;justify-content:center;font-size:.9rem;flex-shrink:0;">
    {{ ['jornal'=>'📅','quincenal'=>'📆','mensual'=>'🗓️','bono'=>'🎁','anticipo'=>'⏩'][$pg->tipo_pago] ?? '💵' }}
  </div>
  <div style="flex:1;min-width:0;">
    <div style="font-size:.87rem;font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;">{{ $pgConcepto }}</div>
  ...
  <div style="display:flex;flex-direction:column;align-items:flex-end;flex-shrink:0;gap:2px;">
    <div style="font-weight:800;color:var(--verde-dark);white-space:nowrap;">${{ number_format($pg->valor,0,',','.') }}</div>
...
document.querySelector('[name="tipo_pago"]').addEventListener('change', function() {
  ...
  if (tipo === 'jornal') {
    ...
    if (dias > 0) valInput.value = Math.round(dias * {{ $persona->valor_jornal }});
  }
  @endif
  @if($persona->valor_mensual)
  if (tipo === 'mensual') {
    valInput.value = {{ $persona->valor_mensual }};
  }
  if (tipo === 'quincenal') {
    valInput.value = Math.round({{ $persona->valor_mensual ?? 0 }} / 2);
  }
  @endif
});
