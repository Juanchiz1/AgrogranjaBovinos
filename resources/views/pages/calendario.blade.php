...
<div style="background:var(--surface);border-radius:var(--radius-lg);padding:14px;margin-bottom:16px;box-shadow:var(--shadow-sm);">
  ...
  <div class="calendar-grid">
    ...
    @for($d=1; $d<=$diasEnMes; $d++)
      ...
      <div class="{{ $cls }}" onclick="abrirDia('{{ $ds }}')">{{ $d }}</div>
    @endfor
  </div>
</div>
@endif
...
var tareasPorDia = {};
@foreach($tareas as $t)
@php $fecha = substr($t->fecha, 0, 10); @endphp
if (!tareasPorDia['{{ $fecha }}']) tareasPorDia['{{ $fecha }}'] = [];
tareasPorDia['{{ $fecha }}'].push({
  id:        {{ $t->id }},
  titulo:    {!! json_encode($t->titulo) !!},
  tipo:      {!! json_encode($t->tipo ?? 'otro') !!},
  prioridad: {!! json_encode($t->prioridad ?? 'media') !!},
  hora:      {!! json_encode($t->hora ? substr($t->hora,0,5) : null) !!},
  completada:{{ $t->completada ? 'true' : 'false' }},
  cultivo:   {!! json_encode($t->cultivo_nombre ?? null) !!},
  animal:    {!! json_encode(($t->animal_nombre ?? null) ? ($t->animal_especie.' - '.$t->animal_nombre) : null) !!},
  responsable:{!! json_encode($t->responsable ?? null) !!},
});
@endforeach
