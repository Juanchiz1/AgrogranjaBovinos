...
<div class="header">
  ...
  <div class="header-title">Reporte de Gastos {{ $anio }}</div>
...
<div class="kpis">
  <div class="kpi marron">
    <div class="k-val">${{ number_format($totalAnio,0,',','.') }}</div>
    <div class="k-lbl">Total gastos {{ $anio }}</div>
  </div>
  <div class="kpi gris">
    <div class="k-val">{{ $gastos->count() }}</div>
    <div class="k-lbl">Registros</div>
  </div>
  <div class="kpi gris">
    <div class="k-val">{{ $porCategoria->count() }}</div>
    <div class="k-lbl">Categorías</div>
  </div>
  <div class="kpi gris">
    <div class="k-val">${{ number_format($gastos->count() > 0 ? $totalAnio / $gastos->count() : 0, 0, ',', '.') }}</div>
...
<div class="two-col">
  <div class="col-cat">
    ...
    <table class="cat-table">
      ...
      <tbody>
        @foreach($porCategoria->sortByDesc(fn($v) => $v)->take(12) as $cat => $total)
        @php $pct = $totalAnio > 0 ? round($total/$totalAnio*100,1) : 0; @endphp
        <tr>
          <td>
            {{ $cat }}
...
<div class="section-full">
  ...
  <table class="detail-table">
    ...
    <tbody>
      ...
      <tr>
        <td style="color:#cbd5e1;font-family:sans-serif;">{{ $i+1 }}</td>
        <td style="font-family:sans-serif;white-space:nowrap;color:#64748b;">
          {{ $g->fecha ? \Carbon\Carbon::parse($g->fecha)->format('d/m/Y') : '—' }}
        </td>
        <td>
          <div class="desc-text">{{ $g->descripcion }}</div>
          @if($g->cantidad)
          <div class="meta-text">{{ $g->cantidad }} {{ $g->unidad_cantidad }}</div>
          @endif
        </td>
        <td><span class="cat-chip">{{ $g->categoria }}</span></td>
        <td style="color:#64748b;font-size:10px;">
          {{ $g->proveedor ?? '' }}
          @if($g->cultivo_nombre)
            @if($g->proveedor) · @endif
            <span style="color:#15803d;">{{ $g->cultivo_nombre }}</span>
          @endif
          @if(!$g->proveedor && !$g->cultivo_nombre) — @endif
        </td>
        <td class="r">${{ number_format($g->valor,0,',','.') }}</td>
      </tr>
      @endforeach
    </tbody>
    <tfoot>
      <tr>
        <td colspan="5">Total gastos {{ $anio }}</td>
        <td class="r">${{ number_format($totalAnio,0,',','.') }}</td>
      </tr>
    </tfoot>
  </table>
</div>
...
<div class="footer">
  <div><strong>Agrogranja</strong> — Sistema de Gestión para Fincas &nbsp;|&nbsp; {{ $gastos->count() }} registros · {{ $porCategoria->count() }} categorías</div>
