...
<div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid var(--border);">
  <div style="width:34px;height:34px;border-radius:50%;background:var(--verde-bg);display:flex;align-items:center;justify-content:center;font-size:.9rem;flex-shrink:0;">
    <?php echo e(['jornal'=>'📅','quincenal'=>'📆','mensual'=>'🗓️','bono'=>'🎁','anticipo'=>'⏩'][$pg->tipo_pago] ?? '💵'); ?>

  </div>
  <div style="flex:1;min-width:0;">
    <div style="font-size:.87rem;font-weight:600;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;"><?php echo e($pgConcepto); ?></div>
  ...
  <div style="display:flex;flex-direction:column;align-items:flex-end;flex-shrink:0;gap:2px;">
    <div style="font-weight:800;color:var(--verde-dark);white-space:nowrap;">$<?php echo e(number_format($pg->valor,0,',','.')); ?></div>
    <form method="POST" action="<?php echo e(route('personas.pago.delete',[$persona->id,$pg->id])); ?>" onsubmit="return confirm('Eliminar?')">
      <?php echo csrf_field(); ?>
      <button type="submit" class="btn btn-sm btn-ghost" style="font-size:.7rem;padding:1px 6px;color:var(--text-muted);">✕ Eliminar</button>
    </form>
  </div>
</div>
    ...
    <form method="POST" action="<?php echo e(route('personas.pago.store',$persona->id)); ?>">
      ...
      <div class="form-group">
        ...
        <select name="tipo_pago" class="form-control" required>
          <option value="jornal">📅 Jornal (por días)</option>
<option value="quincenal">📆 Quincenal (15 días)</option>
<option value="mensual">🗓️ Mensual</option>
<option value="bono">🎁 Bono / Prima</option>
...
<script>
// Auto-cálculo según tipo de pago
document.querySelector('[name="tipo_pago"]').addEventListener('change', function() {
  var tipo = this.value;
  var valInput = document.getElementById('valorPago');
  <?php if($persona->valor_jornal): ?>
  if (tipo === 'jornal') {
    var dias = parseFloat(document.getElementById('diasPago').value) || 0;
    if (dias > 0) valInput.value = Math.round(dias * <?php echo e($persona->valor_jornal); ?>);
  }
  <?php endif; ?>
  <?php if($persona->valor_mensual): ?>
  if (tipo === 'mensual') {
    valInput.value = <?php echo e($persona->valor_mensual); ?>;
  }
  if (tipo === 'quincenal') {
    valInput.value = Math.round(<?php echo e($persona->valor_mensual ?? 0); ?> / 2);
  }
  <?php endif; ?>
});
...
document.getElementById('diasPago').addEventListener('input', function() {
  var tipo = document.querySelector('[name="tipo_pago"]').value;
  if (tipo !== 'jornal') return;
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\Users\Juan Diego\Documents\Universidad Documentos clases\Desarrollo Web 2\AgroGranja Bovinos\AgrogranjaBovinos\resources\views/pages/persona-detalle.blade.php ENDPATH**/ ?>