...
<div style="background:var(--surface);border-radius:var(--radius-lg);padding:14px;margin-bottom:16px;box-shadow:var(--shadow-sm);">
  ...
  <div class="calendar-grid">
    ...
    <?php for($d=1; $d<=$diasEnMes; $d++): ?>
      ...
      <div class="<?php echo e($cls); ?>" onclick="abrirDia('<?php echo e($ds); ?>')"><?php echo e($d); ?></div>
    <?php endfor; ?>
  </div>
</div>
<?php endif; ?>
...
<div class="modal-overlay" id="modalDia" style="display:none;">
  <div class="modal-sheet">
    <div class="modal-handle"></div>
    <h3 class="modal-title" id="modalDiaTitulo">📅 Tareas del día</h3>
    <div id="modalDiaContenido" style="max-height:60vh;overflow-y:auto;"></div>
    <div class="flex gap-2 mt-3">
      <button type="button" class="btn btn-ghost btn-full"
              onclick="closeModal('modalDia')">Cerrar</button>
      <button type="button" class="btn btn-primary btn-full"
              id="btnNuevaTareaDelDia"
              onclick="closeModal('modalDia');openModal('modalNuevaTarea')">
        + Nueva tarea
      </button>
    </div>
  </div>
...
<script>
// Datos de tareas del mes inyectados desde Blade
var tareasPorDia = {};
...
<?php $fecha = substr($t->fecha, 0, 10); ?>
if (!tareasPorDia['<?php echo e($fecha); ?>']) tareasPorDia['<?php echo e($fecha); ?>'] = [];
tareasPorDia['<?php echo e($fecha); ?>'].push({
  id:        <?php echo e($t->id); ?>,
  titulo:    <?php echo json_encode($t->titulo); ?>,
  tipo:      <?php echo json_encode($t->tipo ?? 'otro'); ?>,
  prioridad: <?php echo json_encode($t->prioridad ?? 'media'); ?>,
  hora:      <?php echo json_encode($t->hora ? substr($t->hora,0,5) : null); ?>,
  completada:<?php echo e($t->completada ? 'true' : 'false'); ?>,
  cultivo:   <?php echo json_encode($t->cultivo_nombre ?? null); ?>,
  animal:    <?php echo json_encode(($t->animal_nombre ?? null) ? ($t->animal_especie.' - '.$t->animal_nombre) : null); ?>,
  responsable:<?php echo json_encode($t->responsable ?? null); ?>,
});
<?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
...
function abrirDia(fecha) {
  ...
  // Título del modal
  var label = dia + ' de ' + mesesEs[mes] + ' ' + anio;
  if (fecha === hoy) label = '📍 Hoy — ' + label;
  document.getElementById('modalDiaTitulo').textContent = '📅 ' + label;

  // Pre-llenar la fecha en el modal de nueva tarea
  var inputFecha = document.querySelector('#modalNuevaTarea input[name="fecha"]');
  if (inputFecha) inputFecha.value = fecha;

  var tareas  = tareasPorDia[fecha] || [];
  var html    = '';

  if (tareas.length === 0) {
    html = '<div style="text-align:center;padding:24px;color:#94a3b8;">'
         + '<div style="font-size:2rem;margin-bottom:8px;">📭</div>'
         + '<p style="font-size:.88rem;">Sin tareas para este día.</p>'
         + '</div>';
  } else {
    var colPrioridad = {alta:'#fef2f2', media:'#fffbeb', baja:'#f0fdf4'};
    var txtPrioridad = {alta:'🔴 Alta', media:'🟡 Media', baja:'🟢 Baja'};

    tareas.forEach(function(t) {
      var bg     = t.completada ? '#f8fafc' : (colPrioridad[t.prioridad] || '#f8fafc');
      var decos  = t.completada ? 'text-decoration:line-through;color:#94a3b8;' : '';
      var chips  = '';
      if (t.cultivo)    chips += '<span style="background:#dcfce7;color:#15803d;border-radius:20px;padding:2px 8px;font-size:.7rem;">🌱 '+t.cultivo+'</span> ';
      if (t.animal)     chips += '<span style="background:#ede9fe;color:#6d28d9;border-radius:20px;padding:2px 8px;font-size:.7rem;">🐄 '+t.animal+'</span> ';
      if (t.responsable)chips += '<span style="background:#e0f2fe;color:#0369a1;border-radius:20px;padding:2px 8px;font-size:.7rem;">👤 '+t.responsable+'</span>';

      html += '<div style="background:'+bg+';border-radius:10px;padding:10px 12px;margin-bottom:8px;">'
            + '<div style="font-size:.88rem;font-weight:600;'+decos+'">'+t.titulo+'</div>'
            + '<div style="font-size:.75rem;color:#64748b;margin-top:3px;">'
            + (t.hora ? '⏰ '+t.hora+' · ' : '')
            + txtPrioridad[t.prioridad]
            + (t.completada ? ' · ✅ Completada' : '')
            + '</div>'
            + (chips ? '<div style="margin-top:6px;display:flex;flex-wrap:wrap;gap:4px;">'+chips+'</div>' : '')
            + '</div>';
    });
  }

  document.getElementById('modalDiaContenido').innerHTML = html;
  openModal('modalDia');
}
</script>
<?php $__env->stopPush(); ?>
<?php $__env->stopSection(); ?>
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\Users\Juan Diego\Documents\Universidad Documentos clases\Desarrollo Web 2\AgroGranja Bovinos\AgrogranjaBovinos\resources\views/pages/calendario.blade.php ENDPATH**/ ?>