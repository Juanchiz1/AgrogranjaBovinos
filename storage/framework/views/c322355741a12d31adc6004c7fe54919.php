...
<link rel="stylesheet" href="<?php echo e(asset('css/gastos.css')); ?>">
<?php $__env->stopPush(); ?>
...
<div class="form-group">
  <label style="display:flex;align-items:center;gap:8px;cursor:pointer;">
    <input type="checkbox" name="agregar_inventario" value="1" id="chkInvGasto"
           onchange="document.getElementById('wrapInvGasto').style.display=this.checked?'block':'none'"
           style="width:16px;height:16px;">
    📦 ¿Agregar este insumo al inventario?
  </label>
</div>
<div id="wrapInvGasto" style="display:none;border:1px solid #e2e8f0;border-radius:10px;padding:12px;margin-bottom:10px;">
  <div style="font-size:.75rem;font-weight:700;color:#475569;margin-bottom:8px;">DATOS DEL INSUMO</div>
  <div class="form-group">
    <label>Nombre del insumo *</label>
    <?php if(isset($inventarioItems) && $inventarioItems->count()): ?>
    <select name="inv_nombre" class="form-control" id="selectInvGasto"
            onchange="toggleNombreInv(this)">
      <option value="">— Nuevo insumo —</option>
      <?php $__currentLoopData = $inventarioItems; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <option value="<?php echo e($item->nombre); ?>"
                data-unidad="<?php echo e($item->unidad); ?>"
                data-stock="<?php echo e($item->cantidad_actual); ?>">
          <?php echo e($item->nombre); ?> (<?php echo e($item->cantidad_actual); ?> <?php echo e($item->unidad); ?>)
        </option>
      <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
    </select>
    <?php endif; ?>
    <input type="text" name="inv_nombre" id="inputNombreInv"
           class="form-control mt-1" placeholder="Ej: Urea 46%, Concentrado bovino...">
  </div>
  <div class="grid-2">
    <div class="form-group">
      <label>Cantidad que ingresa</label>
      <input type="number" name="inv_cantidad" class="form-control" step="any" min="0" placeholder="0">
    </div>
    <div class="form-group">
      <label>Unidad</label>
      <input type="text" name="inv_unidad" id="inputUnidadInv"
             class="form-control" placeholder="kg, bultos, L...">
    </div>
  </div>
  <div class="form-group">
    <label>Stock mínimo (alerta)</label>
    <input type="number" name="inv_stock_minimo" class="form-control" step="any" min="0" placeholder="0">
  </div>
</div>
 

<?php if(isset($trabajadores) && $trabajadores->count()): ?>
<div class="form-group" id="wrapPersonaNomina" style="display:none;">
  <label>👷 ¿A quién se le paga?</label>
  <select name="persona_nomina_id" class="form-control">
    ...
    <?php $__currentLoopData = $trabajadores; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $t): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
      <option value="<?php echo e($t->id); ?>"><?php echo e($t->nombre); ?><?php echo e($t->cargo ? ' · '.$t->cargo : ''); ?></option>
    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
  </select>
  <div style="font-size:.72rem;color:#64748b;margin-top:2px;">
    Se registrará automáticamente en el módulo de Personas.
  </div>
</div>
<?php endif; ?>
...
<div class="modal-overlay" id="modalNuevoRecurrente" style="display:none;">
  <div class="modal-sheet"><div class="modal-handle"></div><h3 class="modal-title">🔄 Gasto recurrente</h3>
    ...
    <form method="POST" action="<?php echo e(route('gastos.recurrente.store')); ?>"><?php echo csrf_field(); ?>
      ...
      <?php if(isset($trabajadores) && $trabajadores->count()): ?>
<div class="form-group">
  <label>👷 Asociar a trabajador (nómina)</label>
  <select name="persona_nomina_id" class="form-control">
    <option value="">— Sin asignar —</option>
    <?php $__currentLoopData = $trabajadores; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $t): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
      <option value="<?php echo e($t->id); ?>"><?php echo e($t->nombre); ?><?php echo e($t->cargo ? ' · '.$t->cargo : ''); ?></option>
  ...
  <div style="font-size:.72rem;color:#64748b;margin-top:2px;">
    Al registrar este recurrente se creará automáticamente un pago en Personas.
  </div>
</div>
<?php endif; ?>
...
// Mostrar campo trabajador cuando la categoría es mano de obra
document.querySelector('#modalNuevoGasto select[name="categoria"]')
  .addEventListener('change', function() {
    var nomina = ['Mano de obra','Jornales','Administración'];
    var wrap = document.getElementById('wrapPersonaNomina');
    if (wrap) wrap.style.display = nomina.includes(this.value) ? 'block' : 'none';
  });

function toggleNombreInv(sel) {
  var input = document.getElementById('inputNombreInv');
  var unidadInput = document.getElementById('inputUnidadInv');
  if (sel.value && sel.value !== '') {
    input.value = sel.value;
    var opt = sel.options[sel.selectedIndex];
    if (unidadInput) unidadInput.value = opt.getAttribute('data-unidad') || '';
  } else {
    input.value = '';
  }
}
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\Users\Juan Diego\Documents\Universidad Documentos clases\Desarrollo Web 2\AgroGranja Bovinos\AgrogranjaBovinos\resources\views/pages/gastos.blade.php ENDPATH**/ ?>