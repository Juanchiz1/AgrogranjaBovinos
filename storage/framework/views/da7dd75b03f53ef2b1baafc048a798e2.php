...
<?php if($cultivo->faseActual): ?>
<?php
  $faseActual      = $cultivo->faseActual;
  $porcentajeFenol = $cultivo->porcentajeFenologico();
?>
<div style="background:#ffffff;border:1px solid #e2e8f0;border-radius:12px;
     padding:14px 16px;margin-bottom:12px;border-left:4px solid <?php echo e($faseActual->color_hex); ?>;">
  <div style="display:flex;align-items:center;justify-content:space-between;gap:8px;flex-wrap:wrap;">
    <div>
      <div style="font-size:.75rem;color:#64748b;font-weight:600;margin-bottom:2px;">FASE ACTUAL</div>
      <div style="font-weight:800;font-size:1rem;"><?php echo e($faseActual->icono); ?> <?php echo e($faseActual->nombre); ?></div>
      <?php if($cultivo->fecha_cambio_fase): ?>
      <div style="font-size:.75rem;color:#64748b;">
        desde <?php echo e(\Carbon\Carbon::parse($cultivo->fecha_cambio_fase)->format('d/m/Y')); ?>

        · <?php echo e(\Carbon\Carbon::parse($cultivo->fecha_cambio_fase)->diffInDays(now())); ?> días en esta fase
      </div>
      <?php endif; ?>
    </div>
    <a href="<?php echo e(route('cultivos.fenologia', $cultivo->id)); ?>"
  ...
  <div style="margin-top:10px;">
    <div style="display:flex;justify-content:space-between;font-size:.72rem;color:#64748b;margin-bottom:3px;">
      <span>Progreso fenológico</span><span><?php echo e($porcentajeFenol); ?>%</span>
    </div>
    <div style="background:#e2e8f0;border-radius:50px;height:8px;overflow:hidden;">
      <div style="height:100%;background:linear-gradient(90deg,<?php echo e($faseActual->color_hex); ?>,<?php echo e($faseActual->color_hex); ?>CC);
           border-radius:50px;width:<?php echo e($porcentajeFenol); ?>%;transition:width .6s;"></div>
    </div>
  </div>
</div>
...
<div style="background:#fde8e8;border:1px solid #f5c6cb;border-radius:10px;padding:10px 14px;margin-bottom:12px;">
  ...
  <div style="font-size:.8rem;color:#721c24;">
    🧪 <strong><?php echo e($alerta->producto_nombre); ?></strong>
...
<div style="background:#edf7ed;border:1px dashed #3d8b3d;border-radius:12px;
  ...
  <a href="<?php echo e(route('cultivos.fenologia', $cultivo->id)); ?>" class="btn btn-sm btn-primary" style="white-space:nowrap;">
    Asignar fase
  </a>
</div>
<?php endif; ?>
...
<div class="section-card">
  ...
  <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:12px;">
    <div style="background:#edf7ed;border-radius:10px;padding:12px;text-align:center;">
      ...
      <div style="font-weight:800;color:#2d6a2d;">$<?php echo e(number_format($totalIngresos,0,',','.')); ?></div>
    </div>
  ...
  <div style="display:flex;justify-content:space-between;font-size:.85rem;margin-bottom:6px;">
    <span>Balance</span>
    <strong style="<?php echo e($balance >= 0 ? 'color:#2d6a2d' : 'color:var(--rojo)'); ?>">
  ...
  <div style="font-size:.72rem;color:#64748b;margin-top:4px;"><?php echo e($totalGastos > 0 ? round(($totalIngresos/$totalGastos)*100) : 0); ?>% de gastos recuperados</div>
  <?php endif; ?>
</div>
...
<div class="section-card">
  ...
  <form method="POST" action="<?php echo e(route('cultivos.fotos.upload',$cultivo->id)); ?>" enctype="multipart/form-data" id="fotoUploadForm" style="margin-top:12px;display:none;">
    <?php echo csrf_field(); ?>
  ...
  <?php else: ?>
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;padding:8px 0;border-bottom:1px solid #e2e8f0;">
      <div>
        ...
        <div style="font-size:.75rem;color:#64748b;"><?php echo e($g->categoria); ?> · <?php echo e(\Carbon\Carbon::parse($g->fecha)->format('d/m/Y')); ?></div>
      </div>
...
<div class="section-card">
  ...
  <?php else: ?>
    ...
    <div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid #e2e8f0;">
      ...
      <div style="flex:1;">
        <div style="font-size:.88rem;font-weight:<?php echo e($pendiente ? '600' : '400'); ?>;<?php echo e($pendiente ? '' : 'text-decoration:line-through;color:#94a3b8'); ?>"><?php echo e($t->titulo); ?></div>
        <div style="font-size:.75rem;color:#64748b;"><?php echo e(ucfirst($t->tipo)); ?> · <?php echo e(\Carbon\Carbon::parse($t->fecha)->format('d/m/Y')); ?></div>
      </div>
  ...
  <?php else: ?>
    ...
    <div style="display:flex;justify-content:space-between;align-items:center;padding:8px 0;border-bottom:1px solid #e2e8f0;">
      <div>
        ...
        <div style="font-size:.75rem;color:#64748b;"><?php echo e(\Carbon\Carbon::parse($co->fecha_cosecha)->format('d/m/Y')); ?> · <?php echo e(ucfirst($co->calidad)); ?></div>
      </div>
      <div style="text-align:right;">
        <div style="font-weight:700;color:#2d6a2d;"><?php echo e($co->cantidad); ?> <?php echo e($co->unidad); ?></div>
        <?php if($co->valor_estimado): ?><div style="font-size:.75rem;color:#64748b;">$<?php echo e(number_format($co->valor_estimado,0,',','.')); ?></div><?php endif; ?>
      </div>
    </div>
    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
  <?php endif; ?>
...
<div class="section-card">
  ...
  <div style="display:flex;justify-content:space-between;align-items:center;padding:8px 0;border-bottom:1px solid #e2e8f0;">
    <div>
      ...
      <div style="font-size:.75rem;color:#64748b;"><?php echo e($m->motivo ?? 'Sin descripción'); ?> · <?php echo e(\Carbon\Carbon::parse($m->fecha)->format('d/m/Y')); ?></div>
    </div>
...
<div class="section-card">
  ...
  <div class="timeline">
    ...
    <?php $dotClass = 'tipo-'.str_replace('_','-',$item['tipo'] ?? 'nota'); ?>
    <div class="tl-item">
...
<div style="border:1px solid #e2e8f0;border-radius:10px;padding:12px;margin-bottom:12px;">
  <div style="font-size:.8rem;font-weight:700;color:#475569;margin-bottom:10px;">
    🔗 Vincular con otros módulos (opcional)
  </div>
    ...
    <select name="persona_id" class="form-control">
      ...
      <?php $__currentLoopData = $personas; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $per): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <option value="<?php echo e($per->id); ?>">
          <?php echo e($per->nombre); ?><?php echo e($per->cargo ? ' · '.$per->cargo : ''); ?>
  ...
  <?php if(isset($inventarioItems) && $inventarioItems->count()): ?>
  <div class="form-group">
    <label>📦 ¿Consumió un insumo del inventario?</label>
    <select name="inventario_id" class="form-control" id="selectInsumoAvanzado"
            onchange="toggleCantidadInsumo(this.value)">
      <option value="">— Sin consumo de inventario —</option>
      <?php $__currentLoopData = $inventarioItems->groupBy('categoria'); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $cat => $items): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <optgroup label="<?php echo e($cat); ?>">
          <?php $__currentLoopData = $items; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <option value="<?php echo e($item->id); ?>"
                    data-unidad="<?php echo e($item->unidad); ?>"
                    data-stock="<?php echo e($item->cantidad_actual); ?>">
              <?php echo e($item->nombre); ?> (Stock: <?php echo e($item->cantidad_actual); ?> <?php echo e($item->unidad); ?>)
            </option>
          <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
        </optgroup>
      <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
  ...
  <div id="wrapCantidadInsumo" style="display:none;" class="form-group">
    <label>Cantidad usada (<span id="labelUnidadInsumo">unidades</span>)</label>
    <input type="number" name="inventario_cantidad" id="inputCantidadInsumo"
           class="form-control" step="any" min="0.01"
           placeholder="0.00" oninput="calcGastoInsumo()">
    <div style="font-size:.72rem;color:#64748b;margin-top:2px;">
      Stock disponible: <strong id="stockDisponible">—</strong>
    </div>
  </div>
  <?php endif; ?>
  ...
  <div class="form-group">
    <label style="display:flex;align-items:center;gap:8px;cursor:pointer;">
      <input type="checkbox" name="generar_gasto" value="1" id="chkGenerarGasto"
             onchange="document.getElementById('wrapGastoAvanzado').style.display=this.checked?'block':'none'"
             style="width:16px;height:16px;">
      💰 ¿Generar un gasto por esta actividad?
    </label>
  </div>
  <div id="wrapGastoAvanzado" style="display:none;">
    <div class="form-row">
      <div class="form-group">
        <label>Valor del gasto (COP) *</label>
        <input type="number" name="gasto_valor" id="inputGastoValor"
               class="form-control" step="100" min="0" placeholder="0">
      </div>
      <div class="form-group">
        <label>Proveedor / Quién cobró</label>
        <input type="text" name="gasto_proveedor" class="form-control"
               placeholder="Ej: Almacén Agro">
      </div>
    </div>
  </div>
</div>
...
function toggleCantidadInsumo(val) {
  var wrap = document.getElementById('wrapCantidadInsumo');
  if (!wrap) return;
  if (val) {
    wrap.style.display = 'block';
    var sel = document.getElementById('selectInsumoAvanzado');
    var opt = sel.options[sel.selectedIndex];
    document.getElementById('labelUnidadInsumo').textContent = opt.getAttribute('data-unidad') || 'unidades';
    document.getElementById('stockDisponible').textContent   = opt.getAttribute('data-stock') || '—';
  } else {
    wrap.style.display = 'none';
  }
}

function calcGastoInsumo() {
  // Si el checkbox de gasto está marcado, no interferir con el valor manual
  if (document.getElementById('chkGenerarGasto').checked) return;
  // Sugerencia automática: cantidad × precio_unitario del insumo seleccionado
  // (solo si el usuario no ha llenado el campo manualmente)
  var cantInput = document.getElementById('inputCantidadInsumo');
  var gastoInput = document.getElementById('inputGastoValor');
  if (gastoInput && !gastoInput.value && cantInput.value) {
    document.getElementById('chkGenerarGasto').checked = true;
    document.getElementById('wrapGastoAvanzado').style.display = 'block';
  }
}
</script>
<?php $__env->stopPush(); ?>
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH C:\Users\Juan Diego\Documents\Universidad Documentos clases\Desarrollo Web 2\AgroGranja Bovinos\AgrogranjaBovinos\resources\views/pages/cultivo-detalle.blade.php ENDPATH**/ ?>